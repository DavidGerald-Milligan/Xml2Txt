    ...
    {    
        static void Main(string[] args)
        {
            try
            {
                ...
                if (pagesToCreate > 0)
                {
                    Console.WriteLine("Enter the prepend for created files...");
                    string outputFilePrepend = Console.ReadLine();
                    if (!Directory.Exists(outputFilePrepend))
                    {
                        Directory.CreateDirectory(outputFilePrepend);
                    }
                    do
                    {
                                ...
                                if (revisonNode.Name == "revision")
                                {
                                    foreach (XmlNode textNode in revisonNode.ChildNodes)
                                    {
                                        if (textNode.Name == "text")
                                        {
                                            ...
                                            {
                                                string outputFile = outputFilePrepend + "_" + filesCreated++ + ".txt";
                                                CreateFile(outputFilePrepend, outputFile, sb.ToString(), (filesize / 1024 / 1024));
            ...
            LoopEnd:
                ...
                if (pagesProcessed == 0)
                {
                    Console.WriteLine("Nothing created, try again");
                }
                else
                {
                    Console.WriteLine("Processed:" + pagesProcessed);
                    Console.WriteLine("Created:" + filesCreated);
                }
            }
            catch (Exception ex)
            {
        ...
        static void CreateFile(string directory, string outputFile, string content, int size)
        {
            using (FileStream fs = File.Create(directory + "/" + outputFile))
            {
            ...
            Console.WriteLine("Created:" + outputFile + " Size:" + size + "MB");
        }
    }
 }
